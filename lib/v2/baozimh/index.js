...
module.exports = async (ctx) => {
    ...
    const list = $('#layout > div.comics-detail > div:nth-child(3) > div > div:nth-child(4) > div')
        ...
        .map((item) => {
            ...
            if (name === 'wodetudidushidafanpai-yuetian') {
                const parsedLink = new URL(link);
                const chapterSlot = parsedLink.searchParams.get('chapter_slot');
                const colaLink = 'https://www.colamanhua.com/manga-jq58315/1/' + (parseInt(chapterSlot, 10) + 1) + '.html';
                return {
                    title,
                    link,
                    description: `<br/><br/><a href="${link}">点击跳转到包子动漫</a><br/><a href="${colaLink}">点击跳转到考拉动漫</a>`,
                };
            }
